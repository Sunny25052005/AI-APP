...
function resolveType(raw: string | undefined, context: string): FieldType {
  if (!raw) return 'string';
  const lower = raw.toLowerCase() as FieldType;
  if (VALID_TYPES.includes(lower)) return lower;
  console.warn(
    `[Normalizer] Unknown field type "${raw}" on ${context} — defaulting to "string".`
  );
  return 'string';
}
...
function normalizeField(
  ...
  if (typeof raw === 'object' && raw !== null) {
    ...
    if (!name) {
      console.warn(
        `[Normalizer] Field in entity "${entityName}" is missing a "name" property — skipping.`
      );
      return null;
    }
    return {
  ...
  console.warn(
    `[Normalizer] Unexpected field value in entity "${entityName}": ${JSON.stringify(raw)} — skipping.`
  );
  return null;
}
 ...
 */
function normalizeEntity(name: string, raw: RawEntity | null | undefined): EntitySchema | null {
  ...
  const rawFields: RawField[] = Array.isArray(raw.fields) ? raw.fields : [];

  if (!Array.isArray(raw.fields)) {
    console.warn(
      `[Normalizer] Entity "${name}" is missing a "fields" array — creating entity with zero fields.`
    );
  }

  const fields: FieldSchema[] = rawFields
 ...
 */
export function normalizeConfig(raw: RawConfig): AppConfig {
  const entities = new Map<string, EntitySchema>();

  if (!raw.entities || typeof raw.entities !== 'object') {
    console.warn(
      '[Normalizer] Config is missing "entities" key — no routes will be generated.'
    );
    return { entities };
  }
  ...
  for (const [entityName, rawEntity] of Object.entries(raw.entities)) {
    ...
    if (entity) {
      ...
      console.log(
        `[Normalizer] Registered entity "${normalizedName}" with ${entity.fields.length} field(s): ` +
